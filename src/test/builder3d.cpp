...
void Builder3D::buildMaterials() {
    ...
    (*envMappedMaterial)["envTexture"]->set(envTexture);
}
...
void Builder3D::buildModels() {
    ...
    models.push_back(new AssimpModel(colorProgram,
        "resources/assimp/backpack/backpack.obj", DIFFUSE));
}
...
void Builder3D::draw(float dt, std::set<unsigned int> keysPressed) {
    ...
    (*camera)["viewProjMatrix"]->set(camera->getViewProjMatrix());

    for (auto model : models) {
        model->update();
        (*model)["model"]->set(model->getModelMatrix());
        model->draw();
    }

    envMappedProgram->useShaderProgram();
    (*camera)["rayDirMatrix"]->set(camera->getRayDirMatrix());
    quadMesh->draw();
}